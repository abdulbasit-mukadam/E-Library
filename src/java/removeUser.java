...
public class removeUser extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Remove User | LMS</title>");
            ...
            try {
                ...
                ResultSet rs = st.executeQuery("select count(*) from users where ID =" + ID + "");
                while (rs.next()) {
                    if (rs.getInt(1) == 0) {
                        out.println("<h2>rong ID entered!</h2>");
                        out.println("<br>");
                        out.println("<h2>Go <a href=\"removeUser.html\">back</a>!</h2>");
                    } else {
                        Statement stmt = conn.createStatement();
                        stmt.executeUpdate("delete from users where ID =" + ID + "");
                        out.println("<h2>User successfully removed!</h2>");
                        out.println("<br>");
